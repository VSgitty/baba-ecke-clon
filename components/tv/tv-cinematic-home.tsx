        ...
        <div className={styles.heroGrid}>
          <div className={styles.heroCopy}>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <h1 className={`${styles.heroTitle} font-display`}>{activeHero.title}</h1>
                <p className={styles.heroDescription}>{activeHero.description}</p>

                <div className={styles.metaRow}>
                  <span>{activeHero.type}</span>
                  <span>{activeHero.year || "Archiv"}</span>
                  <span>{(activeHero.rating || 0).toFixed(1)}/10</span>
                </div>
              </motion.div>
            </AnimatePresence>
              ...
              <div className={styles.queueList}>
                {nextQueue.map(({ item, index }) => (
                  <button
                    ...
                    onClick={() => setActiveIndex((activeIndex + index + 1) % safeHeroItems.length)}
                  >
        ...
        {shelves.map((shelf) => (
          <section key={shelf.id} className={styles.shelfSection}>
            ...
            <div className={styles.posterRow}>
              {shelf.items.slice(0, 10).map((item) => (
